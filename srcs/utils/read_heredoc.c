...
/*   Updated: 2023/06/21 10:20:33 by rghouzra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_delim_expansion(char **line, int	*flag)
{
	...
	if (line == NULL || *line == NULL)
		return (-1);
	packed.lines = ft_alphasplit2(*line, '\0', \
		(t_alphasplit){0, 0, 0, 0, 0, 0, 0, 0, 0});
	remove_quotes_2(line);
	*flag = (my_strings_count(packed.lines) == 1 && \
		packed.lines[0][0] != '\'' && packed.lines[0][0] != '"');
...
char	*read_heredoc(char *delimiter)
{
	...
	x.fd = ft_open(x.name, O_CREAT | O_RDWR | O_TRUNC, 0644, \
		&g_mnsh->exit_status);
	if (ft_fork() == 0)
	{
