...
/*   Updated: 2023/06/21 21:57:06 by yrhiba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	wildcard_expand_word(t_my_list **new_list, char **word)
{
	...
	{
		remove_quotes(word);
		if (my_list_push_back(new_list, my_list_new_elem(my_string_dup(*word),
					free_string)) == -1)
			exit(EXIT_FAILURE);
		return ;
	}
	...
	else if (my_list_push_back(new_list, my_list_new_elem(my_string_dup(*word),
				free_string)) == -1)
		exit(EXIT_FAILURE);
}
...
static void	wildcard_expand_node(t_my_list **new_list, char **s)
{
	...
	if (my_strings_count(words) != 1)
	{
		...
		remove_quotes_2(s);
		free(tmp);
		if (my_list_push_back(new_list, my_list_new_elem(my_string_dup(*s),
					free_string)) == -1)
			exit(EXIT_FAILURE);
